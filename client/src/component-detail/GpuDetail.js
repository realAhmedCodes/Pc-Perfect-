...
export const GpuDetail = ({ showGpuFunc, GpuValue }) => {
  ...
  //Close Button state
  const [closeBtn, setCloseBtn] = useState(GpuValue);
  ...
  useEffect(() => {
    if (build === "Custom Build") {
      let filteredGpuList = gpus;

      if (gpuBrand !== "Any") {
        filteredGpuList = gpus.filter(
          (gpu) =>
            gpu.price >= gpuPrice1 &&
            gpu.price <= gpuPrice2 &&
            gpu.brand === gpuBrand
      ...
      setFilteredGpu(filteredGpuList);
    }
  }, [build, usage, gpuBrand, gpuPrice1, gpuPrice2, gpus]);

  //console.log(filteredGpu);

  useEffect(() => {
    const selectedGpu = gpus?.find((x) => x.name === modelGpu);

    if (selectedGpu) {
      setGpuData(selectedGpu);
    }
    console.log(selectedGpu);
  }, [filteredGpu, modelGpu]);
  ...
  const checkBtn = () => {
    if (closeBtn === GpuValue) {
      setCloseBtn(!GpuValue);
    }
    showGpuFunc();
  };
